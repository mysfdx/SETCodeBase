({
	...
	resetForm: function(component) {
		component.find('compliant').set('v.value', null);
	...
	validateReasonForChallenge: function(component) {
		var isCompliant = component.find('compliant');
		if (isCompliant.get('v.value')) {
			component.set('v.case.Reason_For_Challenge__c', 'Dealer is compliant with the provisions of TDMC for the month applied above.');
			return true;
		} else {
			component.set('v.case.Reason_For_Challenge__c', 'Dealer is not compliant with the provisions of TDMC for the month applied above.');
		}
